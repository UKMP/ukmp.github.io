window.addEventListener('load', () => {
    document.querySelectorAll('.lg-button').forEach(button => {
        button.addEventListener('mouseover', function (event) {
            // Menunjukkan tooltip
            var tooltip = document.querySelector('.tooltip');
            // cek jika lebar window kurang dari atau sama dengan 775px
            ...
            // Mengatur teks tooltip berdasarkan id tombol
            switch (button.id) {
                case 'draw':
                    tooltip.textContent = 'Draw';
                    break;
                case 'clear':
                    tooltip.textContent = 'Clear';
                    break;
                case 'erase':
                    tooltip.textContent = 'Erase';
                    break;
                case 'select':
    ...
    if (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches) {
        document.querySelector('html').setAttribute('dark-theme', 'dark');
    ...
    if (window.matchMedia && window.matchMedia('(prefers-color-scheme: light)').matches) {
        document.querySelector('html').setAttribute('dark-theme', 'light');
    ...
    const Modl = document.getElementById("modal");
    modalBtn.addEventListener('click', () => {
        modalBg.classList.add('modal-active');
        Modl.classList.add('slide-in');
    });
    modalClose.addEventListener('click', () => {
        modalBg.classList.remove('modal-active');
        Modl.classList.remove('slide-in');
    });
